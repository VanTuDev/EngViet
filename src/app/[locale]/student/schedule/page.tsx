...
export default async function StudentSchedulePage({
  ...
  const [classes, assignments] = await Promise.all([
    ...
    getAssignmentsInRange(from.toISOString(), to.toISOString(), "enrolled"),
  ]);
  ...
  return (
    <>
      <PageHeader title={t("title")} description={t("studentDesc")} />
      <Alert type="info" showIcon icon={<InfoCircleOutlined />} message={t("hint")} className="!items-center" />
      <ScheduleWeekNav from={from} to={to} weekOffset={weekOffset} basePath="/student/schedule" />
